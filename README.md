...
Start redis-service (official redis:7-alpine image).
Start app, which connects to Redis using the service name redis-service — the same kind of DNS name Kubernetes gives you for a Service, so the code doesn't need to change later.
